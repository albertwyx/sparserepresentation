function [B, S] = reg_sparse_coding(X, num_bases, Sigma, beta, gamma, num_iters, batch_size, initB, fname_save)
%
...
%         S = L1QP_FeatureSign_Set(Xb, B, Sigma, pars.beta, pars.gamma);
        S= l1ls_featuresign(B, Xb, pars.gamma);
%         A = B'*B;
%         Q = B'*Xb;
%         for j = 1:size(S,2)
%             index = find(S(:,j)~=0);  %%%  
%             if ~isempty(index)
%                 S(index,j) = A(index,index)\Q(index,j);%%%
%             end
%         end    
        ...
        fprintf('error = %0.7f', norm(Xb-B*S,'fro')/norm(Xb,'fro'));
    end
