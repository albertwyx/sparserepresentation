function Xout = l1ls_featuresign_kernel (AtY, AtA, gamma, Xinit)
...
AtA = double(AtA);
AtY = double(AtY);
Xout= zeros(size(AtA,2), size(AtY,2));
% AtA = A'*A;
% AtY = A'*Y;
...
for i=1:size(AtY,2)
%     if mod(i, 5000)==0, fprintf('.'); end %fprintf(1, 'l1ls_featuresign: %d/%d\r', i, size(Y,2)); end
...
%     else
        [Xout(:,i)]= ls_featuresign_sub ( AtA, AtY(:, i), gamma);
%     end
end
% fprintf(1, '\n');
...
function [x] = ls_featuresign_sub (AtA, Aty, gamma, xinit)
...
%             warning('Maximum number of iteration reached. The solution may not be optimal');
        ...
        [x, theta, act, act_indx1, optimality1, lsearch, fobj] = compute_FS_step (x, AtA, Aty, theta, act, act_indx1, gamma);
...
fobj = fobj_featuresign(x, AtA, Aty, gamma);
...
function [x, theta, act, act_indx1, optimality1, lsearch, fobj] = compute_FS_step (x, AtA, Aty, theta, act, act_indx1, gamma)
...
a = 0.5*((x_new-x2)'*AtA(act_indx1,act_indx1)*(x_new-x2));
...
function [f, g] = fobj_featuresign(x, AtA, Aty, gamma)
