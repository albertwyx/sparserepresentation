function linear_libsvm_classification_1v1(tr_max_fea,tr_label,ts_max_fea,ts_label,resultmaxpathspm,C)
...
% end
model = svmtrain(tr_label,[(1:length(tr_label))',double(kernel_train)],options);
clear kernel_train;   
...
[predict_label] = svmpredict(ts_label,[(1:length(ts_label))',double(kernel_test)],model);
acc = zeros(nclass, 1);
